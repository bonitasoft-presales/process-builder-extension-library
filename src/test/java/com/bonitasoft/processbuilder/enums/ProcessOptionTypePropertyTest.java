...
/**
 * Property-based tests for {@link ProcessOptionType} enum.
 * Tests invariants that must hold for any valid input.
 *
 * @author Bonitasoft
 * @since 1.0
 */
@Label("ProcessOptionType Property-Based Tests")
class ProcessOptionTypePropertyTest {
    ...
    void allConstantsShouldHaveNonNullKey(@ForAll @From("processOptionTypes") ProcessOptionType type) {
        assertThat(type.getKey()).isNotNull().isNotBlank();
    }

    @Property(tries = 100)
    @Label("All enum constants should have non-null description")
    void allConstantsShouldHaveNonNullDescription(@ForAll @From("processOptionTypes") ProcessOptionType type) {
        assertThat(type.getDescription()).isNotNull().isNotBlank();
    }

    @Property(tries = 100)
    @Label("Enum ordinal() should be valid index")
    void enumOrdinalShouldBeValidIndex(@ForAll @From("processOptionTypes") ProcessOptionType type) {
        assertThat(type.ordinal())
            .isGreaterThanOrEqualTo(0)
            .isLessThan(ProcessOptionType.values().length);
    }

    @Property(tries = 100)
    @Label("Each constant should be retrievable by name")
    void eachConstantShouldBeRetrievableByName(@ForAll @From("processOptionTypes") ProcessOptionType type) {
        assertThat(ProcessOptionType.valueOf(type.name())).isEqualTo(type);
    }
    ...
    void isValidShouldReturnTrueForValidEnumNames(@ForAll @From("processOptionTypes") ProcessOptionType type) {
        assertThat(ProcessOptionType.isValid(type.name())).isTrue();
        assertThat(ProcessOptionType.isValid(type.name().toLowerCase())).isTrue();
        assertThat(ProcessOptionType.isValid(type.name().toUpperCase())).isTrue();
    }

    @Property(tries = 100)
    @Label("isValid should handle spaces around valid names")
    void isValidShouldHandleSpacesAroundValidNames(@ForAll @From("processOptionTypes") ProcessOptionType type) {
        assertThat(ProcessOptionType.isValid("  " + type.name() + "  ")).isTrue();
    }
    ...
    void isValidShouldReturnFalseForInvalidStrings(
        ...
        for (ProcessOptionType type : ProcessOptionType.values()) {
        ...
        if (!isValidName) {
            assertThat(ProcessOptionType.isValid(input)).isFalse();
        }
    }
    ...
    void isValidShouldReturnFalseForNull() {
        assertThat(ProcessOptionType.isValid(null)).isFalse();
    }

    @Property(tries = 100)
    @Label("isValid should return false for empty string")
    void isValidShouldReturnFalseForEmpty() {
        assertThat(ProcessOptionType.isValid("")).isFalse();
        assertThat(ProcessOptionType.isValid("   ")).isFalse();
    }
